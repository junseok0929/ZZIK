...
현재 개발 브랜치는 `feat/m01-portable-aws-preflight`이다. 다음 실제 연결은 실행 환경·버킷·역할 및 비용/유지 시간이 확정된 뒤 진행한다. 이전 단계의 테스트 숫자는 당시 결과이므로 최신 전체 개수와 구분한다.

### 환경 변경과 읽기 전용 사전 점검

- 교육 계정 이름·북버지니아 리전 제한을 유지하는 `education`과 리전·계정·역할·버킷을 지정하는 `custom`을 분리했다. 대회 설정은 아직 확정하지 않았다. 두 경로 모두 Linux EC2 역할 인증이 필요하다.
